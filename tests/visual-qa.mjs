...
async function assertCommerceVisibility(page, route) {
  const commerce = await page.evaluate(() => {
    ...
    const broken = cards.map((card, index) => {
      ...
      const media = card.querySelector('.stush-product-card__media'); const image = media?.querySelector('img.stush-product-card__img--primary') || media?.querySelector('img');
      const imageStyle = image ? getComputedStyle(image) : null; const imageRect = image?.getBoundingClientRect();
      return { index, title: card.querySelector('.stush-product-card__name')?.textContent?.trim() || '', opacity:Number.parseFloat(style.opacity||'1'), visibility:style.visibility, display:style.display, width:rect.width, height:rect.height, hasImage:Boolean(image), complete:image?.complete??false, naturalWidth:image?.naturalWidth??0, naturalHeight:image?.naturalHeight??0, currentSrc:image?.currentSrc||image?.src||'', imageOpacity:imageStyle?Number.parseFloat(imageStyle.opacity||'1'):0, imageWidth:imageRect?.width||0, imageHeight:imageRect?.height||0 };
    ...
    const summary = document.querySelector('.collection-browser__summary strong'); const editFloor = document.querySelector('.edit-product-floor');
    return { count:cards.length, broken, expectedShopCount:summary?Number.parseInt(summary.textContent||'0',10):null, expectedEditCount:editFloor?Number.parseInt(editFloor.dataset.productCount||'0',10):null };
  });
...
async function assertEditorialImages(page, route) {
  const result = await page.evaluate(() => {
    ...
    const nodes = [...new Set([...document.querySelectorAll(selectors)])];
    const broken = nodes.map((img,index)=>({ index, alt:img.alt||'', complete:img.complete, naturalWidth:img.naturalWidth, naturalHeight:img.naturalHeight, currentSrc:img.currentSrc||img.src||'', opacity:Number.parseFloat(getComputedStyle(img).opacity||'1'), width:img.getBoundingClientRect().width, height:img.getBoundingClientRect().height })).filter(item=>!item.complete||item.naturalWidth<50||item.naturalHeight<50||!item.currentSrc||item.opacity<.1||item.width<20||item.height<20);
...
async function assertCampaignRefresh(page, route) {
  const required = route === '/' ? ['.flag-campaigns','.home-house-world','.flag-atelier'] : route === '/shop' ? ['.shop-campaign-banner'] : route === '/collections' ? ['.category-landing--v2'] : route === '/lookbook' ? ['.lb-sequence .stush-campaign-art'] : route === '/journal' ? ['.journal-lead.stush-campaign-art'] : route.startsWith('/collections/') ? ['.edit-campaign-interlude'] : [];
  for (const selector of required) {
    const count = await page.locator(selector).count();
    if (count < 1) throw new Error(`${route} missing campaign refresh target ${selector}`);
  }
}
...
async function assertNoSuspiciousBlankBlocks(page, route) {
  const blanks = await page.evaluate(() => {
    ...
    return nodes.map((node,index)=>{ const rect=node.getBoundingClientRect(); const text=node.textContent?.replace(/\s+/g,' ').trim()||''; const media=[...node.querySelectorAll('img,video')].filter(item=>{ const style=getComputedStyle(item); const box=item.getBoundingClientRect(); return style.display!=='none'&&style.visibility!=='hidden'&&Number.parseFloat(style.opacity||'1')>.1&&box.width>20&&box.height>20; }).length; const controls=node.querySelectorAll('a,button,input').length; return {index,className:node.className||node.tagName,height:rect.height,textLength:text.length,media,controls}; }).filter(item=>item.height>320&&item.textLength<4&&item.media===0&&item.controls===0);
  });
...
async function assertHomepageContract(page) {
  await page.evaluate(() => window.scrollTo({top:0,left:0,behavior:'instant'}));
  await page.waitForFunction(() => window.scrollY === 0, null, {timeout:3000}).catch(()=>{}); await page.waitForTimeout(350);
  const hero=page.locator('[data-qa="animation-hero"]'); const postHero=page.locator('[data-qa="post-hero-copy"]'); const nav=page.locator('nav.nav');
  await hero.waitFor({state:'visible'}); await postHero.waitFor({state:'visible'}); await nav.waitFor({state:'visible'});
  const contract=await hero.evaluate(node=>({text:node.textContent?.replace(/\s+/g,' ').trim()||'',prohibitedCount:node.querySelectorAll('h1,h2,h3,h4,p,a,button,span,label,[role="button"]').length,childTags:[...node.children].map(child=>child.tagName.toLowerCase()),videoSrc:node.querySelector('video')?.getAttribute('src')||''}));
  ...
  if(contract.videoSrc!=='/STUSH_VID.mp4') throw new Error(`Homepage hero film changed unexpectedly: ${contract.videoSrc}`);
  const geometry=await page.evaluate(()=>{ const rect=selector=>{const node=document.querySelector(selector);if(!node)return null;const box=node.getBoundingClientRect();return{top:box.top,bottom:box.bottom,width:box.width,height:box.height};}; const mediaNode=document.querySelector('[data-qa="animation-hero"] video'); return{scrollY:window.scrollY,hero:rect('[data-qa="animation-hero"]'),post:rect('[data-qa="post-hero-copy"]'),nav:rect('nav.nav'),media:rect('[data-qa="animation-hero"] video'),objectFit:mediaNode?getComputedStyle(mediaNode).objectFit:null}; });
...
async function assertCategoryIdentity(page, route) {
  ...
  const identity=await page.evaluate(()=>({category:document.querySelector('[data-qa="category-edit"]')?.dataset.category||'',stage:document.querySelectorAll('.edit-stage__piece img').length,code:document.querySelectorAll('.edit-code > div').length,floor:document.querySelectorAll('.edit-product-floor').length,campaign:document.querySelectorAll('.edit-campaign-interlude img').length}));
  if(!identity.category||identity.code<4||identity.floor!==1||identity.campaign!==1) throw new Error(`${route} failed category identity contract: ${JSON.stringify(identity)}`);
}
