...
export default function ProductInteractive({ product, descriptionHtml }) {
  ...
  const optionNames = (product.options || []).filter(option => option.name !== 'Title');
  const firstAvailable = variants.find(variant => variant.available !== false) || variants[0];
  const images = useMemo(() => getCommerceGalleryImages(product), [product]);
  const [selected, setSelected] = useState(() => {
    const initial = {};
    optionNames.forEach((option, index) => {
      initial[option.name] = firstAvailable?.[`option${index + 1}`] || option.values[0];
    });
    return initial;
  });
    ...
    <div className="pdp">
        ...
        {images.length > 1 && (
          <div className="pdp__thumb-grid">
            {images.slice(0, 6).map((img, index) => {
              ...
              return (
                <button
                  key={img.id || img.mediaId || index}
                  onClick={() => setMainImgIdx(index)}
                  aria-label={`Show image ${index + 1}`}
                  aria-pressed={index === mainImgIdx}
                  className={`pdp__thumb${index === mainImgIdx ? ' pdp__thumb--active' : ''}`}
                  style={{ position: 'relative' }}
                >
                  <Image src={src} alt="" fill sizes="120px" style={{ objectFit: 'contain', padding: 4 }} />
                </button>
              );
            })}
          </div>
      ...
      <div className="pdp__info">
        ...
        <div className="pdp__details">
          ...
          <div className="meta pdp__details-grid">
